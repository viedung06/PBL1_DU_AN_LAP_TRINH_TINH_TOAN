...
void delete_k_place(List *L, int k);         // Hàm xóa node tại vị trí k
void add_k_place(List *L, date data, int k); // Hàm thêm node tại vị trí k
void clear_input_buffer();                   // Hàm xóa bộ đệm đầu vào
void read_line(char *buf, int size);         // Hàm đọc một dòng từ đầu vào
int get_days(int in, int out);               // Hàm tính số ngày ở của khách hàng 
char get_room(char *room_number);            // Hàm xác định loại phòng
int get_price(char type);                    // Hàm lấy giá phòng theo loại
void thongke(List *L);                       // Hàm thống kê doanh thu cua khach san
void print_file(List *L);                    // Hàm in thông tin các hóa đơn ra file data.txt
void read_file(List *L);                     // Hàm đọc file input.txt và thêm dữ liệu vào danh sách liên kết
void searchCMT(List *L, char *CMT);          // Hàm tìm kiếm khách hàng theo Chứng minh thư của khách hàng
void searchRoom(List *L, char *room);        // Hàm tìm kiếm khách hàng theo mã phòng
char *lastName(char *name);                  // Hàm bổ trợ tìm khách hàng tên "Minh"
void popMinh(List *L);                       // Hàm xóa khách hàng có tên "Minh"
void sort_by_date_in(List *L);               // Ham sắp xếp thông tin khách hàng theo ngày vào
...
void export_table(List *L);                  // Hàm xuất bảng sơ đồ phòng hiện tại của khách sạn ra terminal
...
void thongke(List *L) { // Hàm thống kê doanh thu của khách sạn 
...
void thongke_theo_ngay(List *L, int date) { // Hàm thống kê doanh thu của khách sạn 
    char tempA[20], tempB[20], tempKhac[20], tempTatCa[20];
    char tongA_str[20], tongB_str[20], tongKhac_str[20], tongTatCa_str[20];
    int tongA = 0, tongB = 0, tongKhac = 0, tongTatCa = 0;
    Node *p = L->head;
...
void normalize_room_code(char *room) {
    ...
    if (start != room) {
        memmove(room, start, strlen(start) + 1);
    }

    for (char *p = room; *p != '\0'; p++) {
        *p = toupper((unsigned char)*p);
    }
}
int check_room_exist(List *L, char *room){ // Hàm kiểm tra xem phòng có tồn tại hay không
...
int check_room_conflict(List *L, char *room, int date_in, int date_out){ // Hàm kiểm tra xem phòng tồn tại và đã có khách đặt hay chưa
    if (!check_room_exist(L, room)) {
...
void input_data(List *L, date *d) {
    ...
    while (!booking_done) {
        ...
        while (1) {
            ...
            int room_exists = check_room_exist(L, d->room_type);
            ...
            int room_available = check_room_conflict(L, d->room_type, d->date_in, d->date_out);
...
void read_file(List *L) { // Đọc file input.txt và thêm dữ liệu vào danh sách liên kết
...
void searchRoom(List *L, char *room){
	if(check_empty(L)){
        printf("Danh sach rong!\n");
        return;
    }
...
void DrawTable() { // Hàm vẽ bảng giới thiệu đề tài và thành viên nhóm trong terminal
    ...
    printf("\n");
    getchar(); // Đợi người dùng nhấn phím để tiếp tục
}
